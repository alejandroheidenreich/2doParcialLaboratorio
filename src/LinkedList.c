 ...
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio
 * \return LinkedList* Retorna  (NULL) Error: si el puntero a la listas o la funcion es NULL
                                ( filterArray ) Si ok, la lista filtrada
 */
	...
	if (this != NULL && fn != NULL){
		...
		if ( filterArray != NULL){
			...
			for(int i=0; i<largo; i++){
				pElement = ll_get(this,i);
				if(fn(pElement)==1){
					verify = ll_add(filterArray, pElement);
					if(verify == -1){
						filterArray = NULL;
						break;
					}
...
/// @param fn
/// @return LinkedList* Retorna (NULL) Error: si el puntero a la listas o la funcion es NULL
///                      (mapArray) Si ok, la lista mapeada
LinkedList* ll_map(LinkedList* this, void* (*fn)(void* element)){
	...
	if(this != NULL && fn != NULL){
		mapArray = ll_clone(this);
		if(mapArray != NULL){
			largo = ll_len(this);
			for(int i=0; i<largo; i++){
				pElement = ll_get(mapArray,i);
				rElement=fn(pElement);
				if(rElement != NULL){
					verify = ll_set(mapArray, i, rElement);
					if(verify == -1){
						mapArray = NULL;
						break;
					}
